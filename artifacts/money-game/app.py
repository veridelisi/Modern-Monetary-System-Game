...
def render_balance_sheet(entity_key, state, highlight=False):
    ...
    rows_a = "".join(
        ...
        f'<td style="width:8px;"></td><td></td><td></td>'
        f'</tr>'
        for k, v in assets_items
    )
    rows_l = "".join(
        f'<tr>'
        f'<td></td><td></td><td style="width:8px;"></td>'
        f'<td style="padding:4px 8px;color:#374151;">{k}</td>'
        f'<td style="text-align:right;padding:4px 8px;font-weight:600;color:#DC2626;">${v}</td>'
        f'</tr>'
...
if step_now < len(SCENARIOS):
    ...
    <div style="background:linear-gradient(135deg,#DCFCE7,#D1FAE5);
      ...
      <p style="color:#166534;margin:0;">You now understand how money really works in the modern economy.</p>
    </div>
    """, unsafe_allow_html=True)
...
# ── EXPLANATION (shown after each step) ───────────────────────────────────────
...
with col3:
    ...
    st.markdown("""
    <div style="background:#FAF5FF;border-radius:10px;padding:12px 14px;margin-top:6px;font-size:12px;color:#6D28D9;line-height:1.6;border:1px solid #E9D5FF;">
...
st.markdown('<div style="font-size:1.1em;font-weight:700;margin-bottom:8px;">🗺️ All 10 Steps</div>', unsafe_allow_html=True)

cols = st.columns(5)
for i, s in enumerate(SCENARIOS):
    cur = st.session_state.current_step
    if i < cur:
        icon, bg, txt = "✅", "#E8F5E9", "#2E7D32"
    elif i == cur:
        icon, bg, txt = "▶️", "#E8EAF6", "#1A237E"
    else:
        icon, bg, txt = "🔒", "#F5F5F5", "#9E9E9E"

    with cols[i % 5]:
        st.markdown(f"""
        <div style="background:{bg};border-radius:10px;padding:10px 8px;
                    text-align:center;margin:3px 0;border:1px solid {txt}20;">
          <div style="font-size:1.4em;">{s['emoji']}</div>
          <div style="font-size:10px;font-weight:700;color:{txt};margin-top:2px;">{icon} Step {s['id']}</div>
          <div style="font-size:10px;color:#666;margin-top:1px;line-height:1.3;">
            {s['title'][:24]}{'…' if len(s['title'])>24 else ''}
          </div>
...
st.markdown(
    ...
    "Based on <a href='https://github.com/veridelisi/money' style='color:#bbb;' target='_blank'>veridelisi/money</a> "
    "· Streamlit · Double-entry bookkeeping engine</p>",
    unsafe_allow_html=True,
)
